        ...
        success: function(response)
         {
            ...
            if(jsonData.success==1){
                ...
                $.NotificationApp.send("Exito !", 'Registro disponible para conciliacion en Pendientes.', "bottom-right", "#FFFFFF", "success");
    ...
    function ver_tabla_conciliados(a){
        ...
        var datatable_seguimiento= $('#cobranzas_tabla').DataTable({
            ...
            if(row.Estado=='Rechazado'){
              ...
              render: function (data, type, row) {                       
              if(row.Exportado!=""){
                return `<td ><span class="badge badge-warning-lighten">Exportado</span></br>`+
             ...
             {data:null,
                render: function(data,type,row){
                    ...
                    return '<td class="dtr-control dt-checkboxes-cell"><div class="form-check"><input value="'+row.id_cobranza+'" type="checkbox" class="form-check-input dt-checkboxes" onclick="calcular_total(0)"><label class="form-check-label">&nbsp;</label></div></td>';
                   
                }else if(row.Exportado=="" && row.Estado=="Rechazado"){
                
                    return `<td style="cursor:point"><i onclick="vuelve('${row.id_cobranza}')" class="mdi mdi-18px mdi-reload text-success"></i>`+
