...
$('#btn_conciliar').click(function(){
  ...
  $.ajax({
    type: "POST",
    url: 'control/procesos/php/panel.php',
    data:{'Conciliar':1,'id_cobranza':id,'Numero':Numero,'Nombre':Nombre,'Fecha':Fecha,'Hora':Hora,'Banco':Banco,'Operacion':Operacion,'Observaciones':Observaciones,'Importe':Importe},
    success: function(response)
    {
...
$('#btn_rechazar').click(function(){
    ...
    $.ajax({
      type: "POST",
      url: 'control/procesos/php/panel.php',
      data:{'Rechazar':1,'id_cobranza':id,'Numero':Numero,'Nombre':Nombre,'Fecha':Fecha,'Hora':Hora,'Banco':Banco,'Operacion':Operacion,'Observaciones':Observaciones,'Importe':Importe},
      success: function(response)
      {
      ...
      var jsonData = JSON.parse(response);
        ...
        if(jsonData.success==1){
          ...
          $.NotificationApp.send("Rechazado !", `<p style='cursor:pointer'>Registro Rechazado. <a onclick='cancelar(${id})'>cancelar</a></p>`, "bottom-right", "#FFFFFF", "warning");
      ...
      render: function (data, type, row) {
          if(row.Conciliado==0){
            if(row.AlertaDuplicidad==0){
                return `<td><a style='cursor:pointer' class='action-icon' onclick='conciliar(${row.id})'><i class='mdi mdi-text-box-check-outline text-warning'></i></a>`+
    ...
    $.ajax({
        ...
        var jsonData = JSON.parse(response);
          ...
          if(jsonData.success==1){
            ...
            $.NotificationApp.send("Exito !", `Registro Conciliado. <a onclick='cancelar(${i})' style='cursor:pointer'>cancelar</a> `, "bottom-right", "#FFFFFF", "success");
    ...
    $.ajax({
        ...
        var jsonData = JSON.parse(response);
          ...
          if(jsonData.success==1){            
            ...
            $.NotificationApp.send("Cancelado ", `Cancelamos la conciliación del movimiento.`, "bottom-right", "#FFFFFF", "warning");
...
$("#centermodal").on('show.bs.modal', function () {
    ...
    $.ajax({
        type: "POST",
        url: 'control/procesos/php/panel.php',
        data:{'Duplicados':1,'fecha':Fecha,'noperacion':Operacion,'banco':Banco,'importe':Importe,'id_cobranza':id_cobranza},
        success: function(response)
        {
        ...
        var jsonData = JSON.parse(response);
          ...
          if(jsonData.success==1){            
            ...
            $('#alerta').html(`<div class="alert alert-danger" role="alert"><strong>Alerta Duplicidad ! </strong> Se encontraron ${jsonData.data.length} registros de igual Fecha, Número de Operación y Banco. <a onclick="ver_duplicados(${id_cobranza})" style='cursor:pointer'><b> Abrir </b></a></div>`);
