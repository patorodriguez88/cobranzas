...
function enviarFormulario(){
    ...
    $('#alert_confirmation_body').html('Confirmo el deposito de $ '+importe+' en la cuenta de Dinter del Banco '+banco);
...
$('#alert_confirmation_btn_ok').click(function(){
    ...
    if(name!="" && ncliente!="" && fecha!="" && banco!=""&& noperacion!="" && importe!=""){

      $.ajax({
        type: "POST",
        url: 'procesos/php/function.php',
        data:{'IngresarPago':1,'name':name,'ncliente':ncliente,'fecha':fecha,'banco':banco,'noperacion':noperacion,'importe':importe},
        success: function(response)
        {
            var jsonData = JSON.parse(response);
            $('#loading').modal('hide');            
                ...
                $.ajax({
                    ...
                    success: function(response)
                    {
                        ...
                        $("#standard-modal").on('hidden.bs.modal', function () {
                            ...
                            $('#texto_exito').html('Cargamos tu Pago en nuestro sistema, el numero de registro es el Numero: </b> '+jsonData.idIngreso);
...
if(doc==""){
    ...
    $.ajax({
        ...
        var jsonData = JSON.parse(response);
            ...
            window.location.href = 'https://www.dintersa.com.ar/cobranza/ppp.html';     
